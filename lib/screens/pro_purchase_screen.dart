  ...
  Future<void> _checkProStatus() async {
    ...
    if (user != null) {
      ...
      if (userDoc.exists && userDoc.data() != null) {
        ...
        int limit = 10;
        final adminListingLimit = (data['adminListingLimit'] as num?)?.toInt();
        ...
        if (data['proUntil'] != null) {
          ...
          if (endDateTime.isAfter(DateTime.now())) {
            ...
            if (adminListingLimit == null) {
              limit = proLimit > 0 ? proLimit : 10; // Paket limiti kullanılır
            }
            if (mounted) {
              setState(() {
                _isPro = true;
                _remainingProDays = endDateTime
                    .difference(DateTime.now())
                    .inDays;
                _proListingLimit = limit;
              });
            }
          }
        }

        QuerySnapshot listings = await FirebaseFirestore.instance
            .collection('listings')
        ...
        if (mounted) {
          setState(() {
            _usedListings = listings.docs.length;
            _currentLimit = limit;
          });
        }
      }
        ...
        if (purchaseDetails.status == PurchaseStatus.error) {
          ...
          try {
            await FirebaseFunctions.instanceFor(
              region: 'europe-west1',
            ).httpsCallable('activateProPurchase').call({
            ...
            if (mounted) {
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(SnackBar(content: Text(tr('pro_success_msg'))));
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            const SizedBox(
                              width: 48,
                            ), // Ortalamayı dengelemek için boşluk
                          ],
                        ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildFeatureRow(
                          Icons.rocket_launch,
                          Colors.purple,
                          tr('higher_listing_limits'),
                        ),
                        _buildFeatureRow(
                          Icons.auto_awesome,
                          Colors.orange,
                          tr('pro_gift_showcase'),
                        ),
                        _buildFeatureRow(
                          Icons.workspace_premium,
                          Colors.amber,
                          tr('pro_badge_profile'),
                        ),
                        _buildFeatureRow(
                          Icons.support_agent,
                          Colors.blue,
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
                color: isPopular ? Colors.amber : Colors.grey.shade200,
                width: isPopular ? 2 : 1,
              ),
            ),
            child: Row(
