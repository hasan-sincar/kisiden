...
class ShowcaseListScreen extends StatefulWidget {
  final bool isAuctionMode;
  const ShowcaseListScreen({super.key, this.isAuctionMode = false});
...
class _ShowcaseListScreenState extends State<ShowcaseListScreen> {
  ...
  Future<void> _loadPreferences() async {
    try {
      ...
      setState(() {
        _isGridView = prefs.getBool('showcase_isGridView') ?? false;
  ...
  Future<void> _savePreferences() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool('showcase_isGridView', _isGridView);
      if (_filterCity != null) {
                ...
                ListTile(
                  title: Text(
                    currentPath.isEmpty
                        ? tr('all_reset_filter')
                        : tr(
                            'all_in_path_listings',
                          ).replaceFirst('%s', currentPath),
                    style: LocalFonts.poppins(
                      fontWeight: FontWeight.bold,
                    ...
                    if (_userPosition == null)
                      InkWell(
                        ...
                        child: Padding(
                          ...
                          child: Text(
                            tr('distance_filter_no_permission'),
                            style: LocalFonts.poppins(
                              fontSize: 10,
                              color: Colors.red,
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Row(
          children: [
            Icon(
              widget.isAuctionMode ? Icons.gavel : Icons.stars,
              color: widget.isAuctionMode ? Colors.deepPurple : Colors.amber,
            ),
            const SizedBox(width: 8),
            Expanded(
              child: Text(
                widget.isAuctionMode
                    ? tr('auction_showcase')
                    : tr('all_home_showcase_listings'),
                style: LocalFonts.poppins(
                  fontWeight: FontWeight.bold,
                  fontSize: 18,
                ),
      ...
      body: SafeArea(
        child: StreamBuilder<DocumentSnapshot>(
          ...
          builder: (context, userSnap) {
            ...
            if (widget.isAuctionMode) {
              // Firestore İndeks (Dizin) hatasına takılmamak için sadece aktif olanları çekip Müzayede filtresini aşağıda yapıyoruz
            ...
            return StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                var filteredDocs = snapshot.data!.docs.where((doc) {
                  ...
                  if (widget.isAuctionMode && data['auctionEndTime'] != null) {
                    if (data['isAuction'] != true)
                      return false; // YENİ: Müzayede olmayanları filtrele
                    if ((data['auctionEndTime'] as Timestamp).toDate().isBefore(
                      DateTime.now(),
                    ))
                      return false;
                  }
                ...
                if (filteredDocs.isEmpty)
                  return Center(
                    child: Text(
                      widget.isAuctionMode
                          ? tr('no_ongoing_live_auction')
                          : tr('no_showcase_listing_found'),
